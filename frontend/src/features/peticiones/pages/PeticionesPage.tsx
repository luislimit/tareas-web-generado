...
export function PeticionesPage(){
 ...
 const [actividadFiltro,setActividadFiltro]=useUserStoredState<EstadoActividadFiltro>(currentUserId,'peticiones','actividad','activas')
 const [estadosFiltro,setEstadosFiltro]=useUserStoredState<string[]|null>(currentUserId,'peticiones','estados',null)
 const [categoriasFiltro,setCategoriasFiltro]=useUserStoredState<string[]>(currentUserId,'peticiones','categorias',[])
 const [subcategoriasFiltro,setSubcategoriasFiltro]=useUserStoredState<string[]>(currentUserId,'peticiones','subcategorias',[])
 const [campoFecha,setCampoFecha]=useUserStoredState<keyof Peticion>(currentUserId,'peticiones','campoFecha','fechaAlta')
 ...
 function openStateHistory(row:Peticion){
  setSelected(row)
  setChange({estadoNuevoId:'',usuarioId:currentUserId||row.usuarioId,fechaCambio:dayjs().format('YYYY-MM-DDTHH:mm'),observaciones:''})
 ...
 function openDuplicate(row:Peticion){
  setSelected(row)
  setDuplicateMode(true)
  setForm({
   codigo:'',asunto:row.asunto,descripcion:row.descripcion??'',categoriaId:row.categoriaId,subcategoriaId:row.subcategoriaId,
   usuarioId:row.usuarioId,estadoId:row.estadoId,fechaInicioPrevista:row.fechaInicioPrevista??'',fechaFinPrevista:row.fechaFinPrevista??'',
   fechaInicioReal:row.fechaInicioReal??'',fechaFinReal:row.fechaFinReal??'',horasPrevistas:row.horasPrevistas??0,porcentaje:row.porcentaje??0,
   rutaDocumentos:row.rutaDocumentos??'',activo:row.activo,
  })
 ...
 async function saveChange(){
  if(!selected||!change.estadoNuevoId)return
  const nuevoEstadoId=id(change.estadoNuevoId)
  await m.changeMutation.mutateAsync({id:selected.id,payload:{...change,estadoNuevoId:nuevoEstadoId,usuarioId:id(change.usuarioId)}})
  ...
  setChange({estadoNuevoId:'',usuarioId:currentUserId||selected.usuarioId,fechaCambio:dayjs().format('YYYY-MM-DDTHH:mm'),observaciones:''})
 ...
 return <Box>
  ...
  <EntityDrawer open={stateHistoryOpen} title={`Estado e historial${selected?` · ${selected.codigo}`:''}`} saving={m.changeMutation.isPending} saveLabel="Cambiar estado" saveDisabled={!change.estadoNuevoId||!change.observaciones.trim()} onClose={()=>setStateHistoryOpen(false)} onSave={saveChange}>
   <Stack spacing={2.25}>
    <Box>
     <Typography variant="subtitle2" fontWeight={700} sx={{mb:1}}>Cambiar estado</Typography>
     <Stack spacing={2}>
      <FormControl size="small" required><InputLabel>Nuevo estado</InputLabel><Select label="Nuevo estado" value={change.estadoNuevoId} onChange={e=>setChange({...change,estadoNuevoId:e.target.value})}>{(eq.data??[]).filter(e=>String(e.id)!==String(selected?.estadoId)).map(e=><MenuItem key={e.id} value={String(e.id)}>{e.nombre}</MenuItem>)}</Select></FormControl>
  ...
  <EntityDrawer open={quickImputacionOpen} title={`Nueva imputación${selected?` · ${selected.codigo}`:''}`} saving={imputacionMutations.createMutation.isPending} onClose={()=>setQuickImputacionOpen(false)} onSave={saveQuickImputacion}>
   <Stack spacing={2} sx={{flex:1,minHeight:0}}>
    ...
    <ExpandingTextField label="Descripción" value={quickImputacion.descripcion} onChange={value=>setQuickImputacion(prev=>({...prev,descripcion:value}))}/>
   </Stack>
  </EntityDrawer>
